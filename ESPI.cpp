...
void			ESPI_Process(void)
{
	switch (step)
	{
		case 0 :	// enable override
			PinSet(SYS_OPT_ESPI_DISABLE);	// disable BBB's control over ESPI, tri-stating the bus and pre-set the chip-selects
			step++;
		return;
		...
		case 2 :	// pulse SCK high
			if (cntr)	// bits left ?
				{ PinSet(SYS_OPT_ESPI_SCK); step++; }
			else
				step = 10;
		return;
		case 3 :	// pulse SCK low
			if (cntr--)	// bits left ?
				{ PinClr(SYS_OPT_ESPI_SCK); step--; }
			else
				step = 10;
		return;
