...
{
	return (run_state_t)((config.status & ((1<<STAT_SYSTEM_STATUS1) | (1<<STAT_SYSTEM_STATUS0))) >> STAT_SYSTEM_STATUS0);
}
...
void 	IOPortInit(void)
{
	...
	PinIpt(PORT_C4); PinPue(PORT_C4);
	PinIpt(PORT_C5); PinPue(PORT_C5);
//	PinIpt(PORT_D0); PinPue(PORT_D0);
//	PinIpt(PORT_D1); PinPue(PORT_D1);
	PinIpt(PORT_D2); PinPue(PORT_D2);
//	PinIpt(PORT_D5); PinPue(PORT_D5);
	PinIpt(PORT_D6); PinPue(PORT_D6);


	PinPud(SYS_IOPT_19V_FET); PinIpt(SYS_IOPT_19V_FET);				// switch to input for reading back a live reset event
	PinIpt(SYS_IPT_CFG0); PinPue(SYS_IPT_CFG0);						// activate PU on CFG0 Jumper input
	PinIpt(SYS_IPT_CFG1); PinPue(SYS_IPT_CFG1);						// activate PU on CFG1 Jumper input
	PinOpt(SYS_OPT_Relays_UNMUTE); PinClr(SYS_OPT_Relays_UNMUTE);	// unmute coil/relay OFF
	PinIpt(SYS_IOPT_Relays_MUTE); PinPud(SYS_IOPT_Relays_MUTE);		// switch to input for reading back external PullUp (HW7.1-ID)
	
	PinIpt(SYS_IPT_nPwrSwitch); PinPue(SYS_IPT_nPwrSwitch);			// activate PU on Power Switch input
	PinOpt(SYS_OPT_SystemLed); PinClr(SYS_OPT_SystemLed);			// clear LED

	PinOpt(SYS_OPT_ESPI_DISABLE); PinClr(SYS_OPT_ESPI_DISABLE);		// BBB ESPI override OFF
	PinOpt(SYS_OPT_ESPI_SAP); PinClr(SYS_OPT_ESPI_SAP);				// BBB ESPI SAP force low (defined voltage)
	PinOpt(SYS_OPT_ESPI_SCK); PinClr(SYS_OPT_ESPI_SCK);				// BBB ESPI SAP force low (defined voltage)

	PinIpt(EPC_IPT_nSystemRunning); PinPud(EPC_IPT_nSystemRunning);	// disable PU (not needed)
	PinOpt(EPC_OPT_Pwr); PinClr(EPC_OPT_Pwr);						// ePC power button released

	PinIpt(BBB_IPT_SystemRunning); PinPue(BBB_IPT_SystemRunning);	// enable PU (pin might be floating)
	PinPud(BBB_OPT_ShutDown); PinOpt(BBB_OPT_ShutDown); PinClr(BBB_OPT_ShutDown);	// BBB power button released

	PinOpt(SYS_OPT_5V_FET); PinSet(SYS_OPT_5V_FET);					// 5V FET ON (default)
	...
	PinIpt(BBB_SER_RXD); PinPud(BBB_SER_RXD);
	PinIpt(BBB_SER_TXD); PinPud(BBB_SER_TXD);
}
...
{
#if !DEBUG_LED
	if (on != 0)	PinSet(SYS_OPT_SystemLed);
	else 			PinClr(SYS_OPT_SystemLed);
#endif
}
...
void	LED_A(int8_t on)			// debug-LED "A": ==0 ==> OFF, >0 ==> ON, <0 ==> toggle
{
	if (config.hardware_id < 71)
		return;
	if (on > 0)			PinSet(SYS_OPT_LED_A);
	else if (on == 0)	PinClr(SYS_OPT_LED_A);
	else				PinTgl(SYS_OPT_LED_A);
}
...
void	LED_B(int8_t on)			// debug-LED "B": ==0 ==> OFF, >0 ==> ON, <0 ==> toggle
{
	if (config.hardware_id < 71)
		return;
	if (on > 0)			PinSet(SYS_OPT_LED_B);
	else if (on == 0)	PinClr(SYS_OPT_LED_B);
	else				PinTgl(SYS_OPT_LED_B);
}
...
{
	if (on)	was_in_running_state_RAM = LIVE_PATTERN;
	else	was_in_running_state_RAM = !LIVE_PATTERN;
}
...
void SwitchMainSupply(uint8_t on)
{
	if ( !config.pwr_fet_readback )		// old style hardware must config this I/O-pin as output at all times
	{
		...
		if (on)		PinSet(SYS_IOPT_19V_FET);
		else		PinClr(SYS_IOPT_19V_FET);
		return;
	}
	...
	if (on)
	{
		PinPue(SYS_IOPT_19V_FET);		// Write a "1" to the pin's data register so that it remains high immediately after changing dir to output
		PinOpt(SYS_IOPT_19V_FET); PinSet(SYS_IOPT_19V_FET);
		Wait(10);
	...
	{	 // switch off. NOTE: if the BBB has overriden the supply then this will have no effect (as it's done in hardware)
		...
		PinOpt(SYS_IOPT_19V_FET); PinClr(SYS_IOPT_19V_FET);
		Wait(10);
...
void 	HardwareInit_2(void)	// secondary hardware init that *DOES* need IRQ (timer) functionality
{
	...
	if ( PinGet(SYS_IOPT_Relays_MUTE) )		// ext Pullup on MUTE means 7.1 hardware
	{
		...
		PinIpt(BBB_IPT_HeartBeat_7_1); PinPue(BBB_IPT_HeartBeat_7_1);			// enable PU (pin might be floating)
	...
	else	// not a 7.1 hardware
	{
		...
		PinIpt(BBB_IPT_HeartBeat_7_0); PinPue(BBB_IPT_HeartBeat_7_0);			// enable PU (pin might be floating)


		// now check for either 7.0 or 6.0
		PinOpt(SYS_OPT_Pod0); PinClr(SYS_OPT_Pod0);
		PinIpt(SYS_OPT_Pod1); PinPue(SYS_OPT_Pod1);
		Wait(2);
		// In V7 Pod1 and Pod0 are connected via 10k,
		// so the internal weak pullup cannot pull Pod1 high
		if ( PinGet(SYS_OPT_Pod1) == 0)
		{	// V7 hardware
			PinIpt(SYS_OPT_Pod0); PinPue(SYS_OPT_Pod0);
	...
	PinOpt(SYS_IOPT_Relays_MUTE); PinSet(SYS_IOPT_Relays_MUTE);
