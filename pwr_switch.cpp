...
void PSW_Process(void)
{
	...
	switch ( accu )
	{
		case DEBOUNCE_CNT	: state = +1; break;	// accu topps out   ==> switch/button state := "active"
		case 0				: state = -1; break;	// accu bottoms out ==> switch/button state := "inactive"
		default				: state = 0;			// accu not settled ==> switch/button state := "bouncing, invalid"
	}
